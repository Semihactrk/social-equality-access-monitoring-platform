...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $enlem = $_POST['enlem'];
    $boylam = $_POST['boylam'];
    // --- YENİ EKLENEN KISIM 1: Formdan gelen SDG verisini al ---
    $sdg_kategori = $_POST['sdg_kategori']; 
    // ------------------------------------------------------------
    ...
    if (empty($errors)) {
        // --- YENİ EKLENEN KISIM 2: SQL Sorgusuna sdg_kategori ekle ---
        // VALUES kısmına bir tane daha ? işareti ekledik (toplam 7 oldu)
        $sql = "INSERT INTO raporlar (kullanici_id, baslik, aciklama, enlem, boylam, fotograf_yolu, sdg_kategori) VALUES (?, ?, ?, ?, ?, ?, ?)";
        
        if ($stmt = $conn->prepare($sql)) {
            // --- YENİ EKLENEN KISIM 3: Parametreleri bağla ---
            // "issddss" -> Sondaki 's' harfi yeni eklenen sdg_kategori (string) için
            // En sona $sdg_kategori değişkenini ekledik
    ...
    <div class="container">
        ...
        <form action="submit_report.php" method="post" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select name="sdg_kategori" id="sdg_kategori" style="width: 100%; padding: 8px; border: 1px solid #ccc; border-radius: 4px;">
                    <option value="SDG-11: Sürdürülebilir Şehirler ve Topluluklar">SDG-11: Sürdürülebilir Şehirler ve Topluluklar (Park, Yol, Ulaşım)</option>
                    <option value="SDG-10: Eşitsizliklerin Azaltılması">SDG-10: Eşitsizliklerin Azaltılması (Engelli Erişimi, Ayrımcılık)</option>
                    <option value="SDG-6: Temiz Su ve Sanitasyon">SDG-6: Temiz Su ve Sanitasyon (Su Kesintisi, Kanalizasyon)</option>
                    <option value="SDG-7: Erişilebilir ve Temiz Enerji">SDG-7: Erişilebilir ve Temiz Enerji (Elektrik, Aydınlatma)</option>
                    <option value="SDG-3: Sağlık ve Kaliteli Yaşam">SDG-3: Sağlık ve Kaliteli Yaşam (Çöp, Koku, Hijyen)</option>
                    <option value="Diğer">Diğer</option>
                </select>
                <small style="color: #666;">Bu sorunun hangi küresel hedefi etkilediğini seçiniz.</small>
            </div>
